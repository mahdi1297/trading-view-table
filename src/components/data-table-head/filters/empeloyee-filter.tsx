...
import { NUMBER_OF_EMPLOYEES, RANGE_SLIDER_EMPLOYEE_MAX_VALUE, RANGE_SLIDER_EMPLOYEE_MIN_VALUE } from '../../../constaints';
...
const EmpeloyeeFilterFilterComponent = () => {
  const [value, setValue] = useState<RangeSlider>({
    min: RANGE_SLIDER_EMPLOYEE_MIN_VALUE,
    max: RANGE_SLIDER_EMPLOYEE_MAX_VALUE
  });

  const _context = useContext(dataContext);

  const minInput = useRef<number | string>(RANGE_SLIDER_EMPLOYEE_MIN_VALUE)
  const maxInput = useRef<number | string>(RANGE_SLIDER_EMPLOYEE_MAX_VALUE)
  ...
  const loader = () => {
    if (value.min !== RANGE_SLIDER_EMPLOYEE_MIN_VALUE
      || value.max !== RANGE_SLIDER_EMPLOYEE_MAX_VALUE) {
      _context.load();
    }
  }
  ...
  return (
    <div className='volume-filter-wrapper'>
      < TitleComponent text={NUMBER_OF_EMPLOYEES} />
...
export default EmpeloyeeFilterFilterComponent